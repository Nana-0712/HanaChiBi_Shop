...
class ErrorBoundary extends Component<{ children: ReactNode }, { hasError: boolean, error: any }> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="min-h-screen bg-primary-light/5 flex items-center justify-center p-4">
          <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl max-w-md w-full text-center border-4 border-primary-light/20">
            <div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <AlertTriangle className="w-10 h-10 text-red-500" />
            </div>
            <h2 className="text-2xl font-black text-gray-900 mb-4 uppercase italic">Oops! Có lỗi rồi</h2>
            <p className="text-gray-500 font-bold mb-8 leading-relaxed">
              {errorMessage}
            </p>
            <button 
              onClick={() => window.location.reload()}
              className="w-full py-4 rounded-2xl bg-primary-dark text-white font-black shadow-xl hover:scale-105 transition-transform"
            >
              Tải lại trang ✨
            </button>
          </div>
